...
describe('SearchBar', () => {
    ...
    test('should call getInfoVideo when videoURL is provided', async () => {
        const setInfoVideo = vi.fn();

        render(
            <SearchBar videoURL="youtube.com/video" setVideoURL={() => { }} setInfoVideo={setInfoVideo}
            />
        );

        
        await vi.waitFor(() => {
            expect(getInfoVideo).toHaveBeenCalledTimes(1);
            expect(getInfoVideo).toHaveBeenCalledWith('youtube.com/video');
        });
    });
    test('should call setInfoVideo with the result of getInfoVideo', async () => {
        const setInfoVideo = vi.fn();
        const fakeVideoData = { title: 'Mi video', duration: 120 };

        
        vi.mocked(getInfoVideo).mockResolvedValueOnce(fakeVideoData);

        render( <SearchBar videoURL="youtube.com/video" setVideoURL={() => { }} setInfoVideo={setInfoVideo}/> );

        await vi.waitFor(() => {
            expect(setInfoVideo).toHaveBeenCalledWith(fakeVideoData);
        });
    });
    test('should NOT call getInfoVideo when videoURL is empty', () => {
        render(<SearchBar videoURL="" setVideoURL={() => { }} setInfoVideo={() => { }} />);

        expect(getInfoVideo).not.toHaveBeenCalled();
    });
